...
valor s (Equi p q) = valor s (Impl p q) && valor s (Impl q p)
...
-- satisfaz :: Prop -> Bool
-- satisfaz ps = any () (atribs ps)

equiv :: Prop -> Prop -> Bool
equiv p q = tautologia (Conj (Impl p q) (Impl q p))

showProp :: Prop -> String
showProp (Const p)  = show p
showProp (Var p)    = show [p]
...
showProp (Equi p q) = "(" ++ showProp p ++ " <=> " ++ showProp q ++ ")"
